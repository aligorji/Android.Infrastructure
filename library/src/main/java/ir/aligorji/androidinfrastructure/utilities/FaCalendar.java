...
public class FaCalendar extends Calendar
{
    ...
    public FaCalendar(TimeZone zone)
    {
        this(zone, Locale.getDefault());
    }

    public FaCalendar(Locale aLocale)
    {
        this(TimeZone.getDefault(), aLocale);
    }

    public FaCalendar(TimeZone zone, Locale aLocale)
    {
    ...
    public Date getGregorianDate(String jalaliDate, int hour, int min, int sec)
    {
        ...
        cal.clear();
        cal.set(gDate.getYear(), gDate.getMonth(), gDate.getDate(), hour, min, sec);
    ...
    public String getJalaliDateTime(Date gDate)
    {
        ...
        //String jalali = jDate.year + "/" + String.format(Locale.US, "%02d",(jDate.month + 1)) + "/" + String.format("%02d",);

        return String.format(Locale.US, "%04d/%02d/%02d", jDate.year, (jDate.month + 1), jDate.date) + " " +
                String.format(Locale.US, "%02d:%02d:%02d", gCal.get(Calendar.HOUR_OF_DAY), gCal.get(Calendar.MINUTE), gCal.get(Calendar.SECOND));

    }

    public FaCalendar(int year, int month, int dayOfMonth)
    {
        this(year, month, dayOfMonth, 0, 0, 0, 0);
    }

    public FaCalendar(int year, int month, int dayOfMonth, int hourOfDay,
                      int minute)
    {
    ...
    FaCalendar(int year, int month, int dayOfMonth, int hourOfDay,
    ...
    {
        switch (dayOfWeek(FaCalendar.jalaliToGregorian(new YearMonthDate(
                year, 0, 1))))
        {
            case 2:
                dayOfYear++;
